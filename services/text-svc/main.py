"""
FixMyText Text Service
======================
Standalone FastAPI service for LOCAL (non-AI) text transformation tools.
...
logger = logging.getLogger("fixmytext.text-svc")
...
async def lifespan(app: FastAPI):
    ...
    await init_redis()
    logger.info("text-svc ready")
    yield

    logger.info("text-svc shutting down …")
    import sentry_sdk
...
app.add_middleware(RequestLoggingMiddleware, logger_name="fixmytext.text-svc")
...
async def health_check():
    ...
    return {"status": "ok", "version": settings.VERSION, "service": "text-svc"}
